...
export async function seedChordTypesIfEmpty(): Promise<void> {
  ...
  if (count > 0) return
  await prisma.chordType.createMany({ data: INITIAL_CHORD_TYPES, skipDuplicates: true })
}
...
export async function getChordLibrary(): Promise<ChordLibrary> {
  const [chordTypes, chords] = await Promise.all([
    ...
    prisma.chord.findMany({ include: { shapes: { orderBy: { orderIndex: 'asc' } } } }),
  ])
  ...
  for (const chord of chords) {
    ...
    library[root]![type] = {
      shapes: chord.shapes.map((s): ChordShape => ({ frets: [s.g, s.c, s.e, s.a] })),
    }
  }
