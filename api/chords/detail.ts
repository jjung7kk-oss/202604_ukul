...
function requireAuth(req: VercelRequest, res: VercelResponse): boolean {
  ...
  if (!verifySessionToken(token)) { res.status(401).json({ error: 'Unauthorized' }); return false }
  return true
}
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const { getChordDetail } = await import('../../shared/chordService.js')
  ...
  if (req.method === 'PUT') {
    ...
    for (const s of shapes) {
      if (!s || typeof s !== 'object' || !('frets' in s)) { res.status(400).json({ error: 'Invalid frets in shapes' }); return }
      const frets = (s as { frets: unknown }).frets
      if (!Array.isArray(frets) || frets.length !== 4 || frets.some((n) => typeof n !== 'number' || n < 0 || n > 15)) {
        res.status(400).json({ error: 'Invalid frets in shapes' }); return
      }
      normalized.push({ frets: [frets[0], frets[1], frets[2], frets[3]] as [number, number, number, number] })
    }
    try {
      const { replaceChordShapes } = await import('../../shared/chordService.js')
