...
function shapeScore(score: {
  ...
  sharedChordText: string; notation: Prisma.JsonValue;
...
export async function saveScore(input: SaveScoreInput) {
  ...
  const sharedChordText = typeof input.sharedChordText === "string" ? input.sharedChordText : "";
  const notationUpdate = input.notation !== undefined ? notationToDb(input.notation) : undefined;
  ...
  if (input.scoreId) {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      await tx.score.update({
        where: { id: input.scoreId },
        data: { title, artist, sharedChordText, ...(notationUpdate !== undefined ? { notation: notationUpdate } : {}) },
      });
